...
var SuperTokens = /** @class */ (function () {
    function SuperTokens(config) {
        ...
        if ((_a = config.experimental) === null || _a === void 0 ? void 0 : _a.plugins) {
            for (var _i = 0, _b = config.experimental.plugins; _i < _b.length; _i++) {
                ...
                if (plugin.compatibleWebJSSDKVersions) {
                    var versionCheck = (0, versionChecker_1.isVersionCompatible)(version_1.package_version, plugin.compatibleWebJSSDKVersions);
                    if (!versionCheck) {
                        throw new Error("Incompatible SDK version for plugin ".concat(plugin.id, ". Version \"").concat(version_1.package_version, "\" not found in compatible versions: ").concat(JSON.stringify(plugin.compatibleWebJSSDKVersions)));
                    }
                }
                if (plugin.dependencies) {
                    var result = plugin.dependencies((0, utils_1.getPublicConfig)(__assign(__assign({}, config), { appInfo: this.appInfo })), finalPluginList.map(utils_1.getPublicPlugin), version_1.package_version);
        ...
        var _loop_1 = function (pluginIndex) {
            ...
            if (plugin.config) {
                // @ts-ignore
                var _c = plugin.config((0, utils_1.getPublicConfig)(__assign(__assign({}, config), { appInfo: this_1.appInfo }))) || {}, appInfo = _c.appInfo, pluginConfig = __rest(_c, ["appInfo"]);
            ...
            if (pluginInit) {
                postSuperTokensInitCallbacks_1.PostSuperTokensInitCallbacks.addPostInitCallback(function () {
                    pluginInit((0, utils_1.getPublicConfig)(__assign(__assign({}, config), { appInfo: _this.appInfo })), _this.pluginList, version_1.package_version);
        ...
        if (!multitenancyFound) {
            this.recipeList.push(recipe_1.Recipe.init()(this.appInfo, config.clientType, enableDebugLogs, overrideMaps));
        }
    }
    /**
