"use strict";
var __assign =
    (this && this.__assign) ||
    function () {
        __assign =
            Object.assign ||
            function (t) {
                for (var s, i = 1, n = arguments.length; i < n; i++) {
                    s = arguments[i];
                    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p)) t[p] = s[p];
                }
                return t;
            };
        return __assign.apply(this, arguments);
    };
Object.defineProperty(exports, "__esModule", { value: true });
exports.generateThirdPartyProviderState =
    exports.getThirdPartyProviderRedirectURL =
    exports.normaliseUserInput =
        void 0;
/* Copyright (c) 2022, VRAI Labs and/or its affiliates. All rights reserved.
 *
 * This software is licensed under the Apache License, Version 2.0 (the
 * "License") as published by the Apache Software Foundation.
 *
 * You may not use this file except in compliance with the License. You may
 * obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
 * License for the specific language governing permissions and limitations
 * under the License.
 */
var normalisedURLPath_1 = require("../../normalisedURLPath");
var utils_1 = require("../authRecipeWithEmailVerification/utils");
function normaliseUserInput(config) {
    var override = __assign(
        {
            functions: function (originalImplementation) {
                return originalImplementation;
            },
            components: {},
        },
        config.override
    );
    return __assign(__assign({}, (0, utils_1.normaliseAuthRecipeWithEmailVerificationConfig)(config)), {
        override: override,
    });
}
exports.normaliseUserInput = normaliseUserInput;
function getThirdPartyProviderRedirectURL(_a) {
    var providerId = _a.providerId,
        appInfo = _a.appInfo;
    var domain = appInfo.websiteDomain.getAsStringDangerous();
    var callbackPath = new normalisedURLPath_1.default("/callback/".concat(providerId));
    var path = appInfo.websiteBasePath.appendPath(callbackPath).getAsStringDangerous();
    return "".concat(domain).concat(path);
}
exports.getThirdPartyProviderRedirectURL = getThirdPartyProviderRedirectURL;
function generateThirdPartyProviderState() {
    // Generate state using algorithm described in https://github.com/supertokens/supertokens-auth-react/issues/154#issue-796867579
    return "".concat(1e20).replace(/[018]/g, function (c) {
        return (parseInt(c) ^ (crypto.getRandomValues(new Uint8Array(1))[0] & (15 >> (parseInt(c) / 4)))).toString(16);
    });
}
exports.generateThirdPartyProviderState = generateThirdPartyProviderState;
