import { doesEmailExist, signIn, signUp } from "supertokens-web-js/recipe/emailpassword";
...
async function appleSignInClicked() {
    try {
        const authUrl = await getAuthorisationURLWithQueryParamsAndSetState({
            ...
            redirectURIOnProviderDashboard: "http://localhost:3000/auth/callback/apple", // This URL goes on the Apple's dashboard
        });
...
async function signUpClicked(email: string, password: string, navigate: (path: string) => void) {
    try {
...
async function signInClicked(email: string, password: string, navigate: (path: string) => void) {
    try {
...
function Auth() {
    ...
    return (
        <div class="wrapper">
            <div class="form-wrap">
                <Show when={showOAuthLoading()}>Logging-in via {showOAuthLoading()}, please wait...</Show>
                <Show when={!showOAuthLoading()}>
                    ...
                    <button onClick={handleGoogleSignInClicked}>Sign-in with Google</button>
                    <button onClick={handleGithubSignInClicked}>Sign-in with GitHub</button>
