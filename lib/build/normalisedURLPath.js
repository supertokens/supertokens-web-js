...
var NormalisedURLPath = /** @class */ (function () {
    function NormalisedURLPath(url, _a) {
        var _b = _a === void 0 ? { includeSearch: false } : _a,
            includeSearch = _b.includeSearch;
        var _this = this;
        this.startsWith = function (other) {
            return _this.value.startsWith(other.value);
        };
        this.appendPath = function (other) {
            return new NormalisedURLPath(_this.value + other.value, { includeSearch: _this.includeSearch });
        };
...
function normaliseURLPathOrThrowError(input, includeSearch) {
    input = input.trim();
    try {
        if (!input.startsWith("http://") && !input.startsWith("https://")) {
            throw new Error("Error converting to proper URL");
        }
        var _a = new URL(input),
